...
interface IJobManager {
    ...
    function createJob(uint256 domainId, address creator, string calldata jobDataURI) external;
    ...
    function blacklistNode(uint256 jobId, uint256 nodeId) external;
    function getJob(uint256 jobId) external view returns (JobInfo memory);
    function getJobFunds(uint256 jobId) external view returns (uint256);
    function getJobProviders(uint256 jobId) external view returns (address[] memory);
    function getJobNodes(uint256 jobId) external view returns (uint256[] memory);
}
